...
look at streaming which provides a model for dealing with data whose length is very large (even infinite).
...
1. Unnecessary round-trips.  
   ...
   any pipelining.  This will result in the game being less responsive and less scalable.
...
This change addresses problems (1) and (3):
...
* Half the network latency.  
  ...
  half the previous network latency (i.e. the path from server-to-client, without first the client-to-server needed to
  send the `GetNext` message above).  That is, the average latency is &frac12;xRTT.
...
## Streaming Sequence
Let's first consider what kind of characteristics we'd want from ideal streaming sequence of items.  Then we can try to
employ that ideal and see if it addresses any of the problems we have identified so far.
...
produced and/or delivered incrementally, otherwise it would just be an `Array`.  The streaming nature of the sequence
allows both the producer and the consumer to overcome real and practical limitations on the size of available memory.
Network and serialization protocols must use buffers that fit into available memory.  A serialization of a very large
object tree, for instance, can't be bigger than the largest such buffer.  A streaming sequence, however, can deliver a
set of items that couldn't fit into memory (if they were all materialized at once).  A very large object tree could be
delivered as a streaming traversal, requiring buffers only large enough to handle a single node at a time.  
...
Let's propose: a streaming **Sequence** is actually defined by *two* capabilities that work together to create a single
streaming sequence:

* **Consumer Capability: called `ISequence<T>`**  
  A capability that gives the holder the right to _read_ a sequence of zero or more (possibly infinite) items (of type
  `T`).

* **Producer Capability: called `ISequenceWriter<T>`**  
  A capability that gives the holder the right to _write_ zero or more items (of type `T`) into a sequence.

The reader side of a streaming sequence provides the consumer with four fundamental actions:
...
terminate the sequence with a diagnostic error (for the consumers's information).  And finally, the ability to abruptly
...
Since our message passing system that support Capability Exchange (as we discussed in the [previous post][devlog-post5])
we may exchange of either a read capability or write capability (or both) in the payload of messages (including response
messages).

## Sequence In Action
Let's assume that our message passing system supports streaming sequences (as, of course, [Promise RPC][promise-rpc]
does).  Borrowing the syntax from [Promise RPC][promise-rpc], we'll assume that a streaming sequence is implemented by
two special, system implemented capabilities.  The type `Sequence<T>` implements the consumer capability and the type
`Sequence<T>.Writer` implements the producer capability.  The message passing system is responsible for implementing
buffering, batching, and flow control between these sequence capabilities (within the parameters set by the consumer).
The consumer type `Sequence<T>` also implements C#'s [`IAsyncEnumerator`][async-enumerator] which internally calls the
`ISequence<T>.Read` method once per loop iteration.
...
1. Unnecessary round-trips.  
   ...
   control.  If an item already exists in the client-side buffer, a call to `Read()` is handled locally and promptly
   (i.e. does NOT need to yield during _await_).  Additionally, the server writes items directly into the producer
   capability and the message passing system is responsible for handling buffering, nagling, and coalescing to produce
   efficient batching for asynchronous background transmission that overlaps with both the server and client side
   processing. Like in the section [Push vs Pull](#push-vs-pull) above, the average latency is &frac12;xRTT.
...
2. Additional allocations.  
   ...
   resolved promptly from the client-side buffer.  No `Read` messages are actually sent, the message passing system
   employs push in the background to asyncronously deliver items to the consumer-side buffer.
...
5. No backpressure.  
   ...
   passing system's implementation of flow control is responsible for guaranteeing that the server never exceeds these
   limits. Additionally, the server can observe client backpressure (communicated automatically in the background by the
   message passing system in the upstream direction consumer-to-producer).  In response to observed backpressure, the
   server can pause production, coalesce items, or drop unnecessary or redundant events to reduce traffic until the
   client catches up.
...
To address all of these issues, and for convenience, a specialized implementation called `Stream` is also made available
by most message passing systems.  The `Stream` is logically identical to a `Sequence<byte>`, but with the following
caveats:

1. Batched Writes  
   The `Write` method of `Stream.Writer` accepts a `Memory<byte>` for efficiency (like `Sequence<Memory<byte>>` did),
   but the `Stream` is NOT required to honor or maintain chunk boundaries.  Like a TCP socket, a Pipe, or a file handle,
   the internal buffering of a `Stream` coalesces all bytes into a single continuous sequence with no internal
   boundaries. Flow control is thus allowed to deliver a fraction of a written buffer if that is all the open window
   allows.
2. Non-aligned Batched Reads  
   The `Read` method of `Stream` returns a `Memory<byte>` for efficiency, but it also does not honor the boundaries of
...
implements first-class capabilities for both `Sequence<T>` and `Stream`.  It provides an implementation of streaming
underlying flow control with buffering, nagling, and automatic batching.  However, in this section, I'd like to
highlight some additional special features that may be unique to [Promise RPC][promise-rpc]'s approach to streaming
sequences.
...
1. It's a Promise-type!  
   ...
   it also exhibits other aspects of proxies.  Methods can be pipelined on it, such as setting the flow control
   parameters or posting a `Read` before the Sequence has been resolved remotely.  And the writer has factory methods
   for creating async iterators.
...
2. It's Full Duplex!  
   ...
   {
     Sequence<Item> GetAndSend(Sequence<Item> theseAreForYou);
   }
   ```
   ...
   This method sends two streaming sequences, while receiving one in return.  All of these sequences have flow control
   and lifetime that is independent of each other and independent of the method call to `SoManyStreams`.

3. It's Serializable!  
   In addition to appearing directly as an argument or return value, a `Sequence<T>` can appear as a member of any other
   serializable `[DataContract]` type.
   ...
   {
     Promise<StructWithStreams> GetStructWithStreams(StructWithStreams arg);
     Promise<(Sequence<A> TheGood, Sequence<B> TheBad)> EvenMoreStreams(Sequence<C> one, Sequence<D> two);
   }
   ```
...
4. It's Composable!  
   ...
   {
     Sequence<Sequence<ISet<T>>> GetAllSequencesThatDontContainThemselves();
   }
   ```

## Conclusion
In this post we talked about streaming sequences including `Sequence<T>` and `Stream`.  We examined some of the issues
that streams solve including memory bounding, network latency and traffic minimization, flow control, and backpressure.
Lastly, we looked at some unique characterisitcs of the [Promise RPC library's][promise-rpc] implementation of
streaming sequences.

This is Part 4 of our look at the [MSC][MSC] RPC system.  In the final part, we'll look at channel lifetime, aborts and
cancellation. Until next time, code on!
