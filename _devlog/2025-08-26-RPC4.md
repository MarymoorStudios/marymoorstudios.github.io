...
does or we wouldn't be writing this :smile:).  Borrowing the syntax from [Promise RPC][promise-rpc], we'll assume for
the sake of this discussion that a streaming sequence is implemented by two special, system implemented capabilities.
The type `Sequence<T>` implements the consumer capability and the type `Sequence<T>.Writer` implements the producer
capability.  The message passing system is responsible for implementing buffering, batching, and flow control between
these two capabilities (constrained by the parameters set by the consumer). The consumer type `Sequence<T>` also
implements C#'s [`IAsyncEnumerator`][async-enumerator] which executes a call to the `ISequence<T>.Read` method once per
loop iteration.
...
5. **No backpressure.**  
   ...
   limits. Additionally, the server can observe client backpressure (communicated automatically through out-of-band
   messaging by the message passing system in the upstream direction consumer-to-producer).  In response to observed
   backpressure, the server can pause production, coalesce items, or drop unnecessary or redundant events to reduce
   traffic until the client catches up.
...
To address all of these issues, and for convenience, a specialized implementation we'll call `Stream` is also made
available in most message passing systems.  The `Stream` is logically identical to a `Sequence<byte>`, but with the
following caveats:
...
implements first-class capabilities for both `Sequence<T>` and `Stream`.  It provides an implementation of streaming's
...
3. **It's Serializable!**  
   ...
   {
     Promise<StructWithStreams> GetStructWithStreams(StructWithStreams asAnArg);
     Promise<(Sequence<A> TheGood, Sequence<B> TheBad)> EvenMoreStreams(Sequence<C> AndAlso, Sequence<D> TheUgly);
   }
   ```
...
4. **It's Composable!**  
   ...
   {
     Sequence<Sequence<object>> GetAllSequencesThatDontContainThemselves();
   }
   ```
