...
either the read capability and/or the write capability as they see fit.
...
available in most message passing systems.  `Bytes` is logically identical to a `Sequence<byte>`, but with the following
caveats:

1. **Batched Writes.**  
   The `Write` method of `Bytes.Writer` accepts a `Memory<byte>` for efficiency (like `Sequence<Memory<byte>>` did), but
   `Bytes` is NOT required to honor or maintain chunk boundaries.  Like a TCP socket, a Pipe, or a file handle, the
   internal buffering of `Bytes` coalesces all bytes into a single continuous sequence with no internal boundaries. Flow
   control is thus defined in terms of `byte`s not _chunks_ and is allowed to deliver a fraction of a producer written
   buffer (if that is all that the open window allows).
...
This is Part 4 of our look at the [MSC][MSC] RPC system.  In the next and final part, we'll look at Sessions, session
lifetime, aborts and cancellation.  Until next time, code on!

## Previous
Read the [previous post][devlog-post5] in this series.

## Next
Read the [next post][devlog-post7] in this series.
