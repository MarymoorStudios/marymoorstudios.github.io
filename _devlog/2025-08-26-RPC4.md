...
## Sequence In Action
Let's assume that our message passing system supports streaming sequences (as, of course, [Promise RPC](
https://www.nuget.org/packages/MarymoorStudios.Core.Rpc/) does).  Borrowing the syntax from [Promise RPC](
https://www.nuget.org/packages/MarymoorStudios.Core.Rpc/), we'll assume that a streaming sequence is implemented by two
special, system implemented capabilities.  The type `Sequence<T>` implements the consumer capability and the type
...
To address all of these issues, and for convenience, a specialized implementation called `Stream` is also made
available by most message passing systems.  The `Stream` is logically identical to a `Sequence<byte>`, but with the following caveats:
