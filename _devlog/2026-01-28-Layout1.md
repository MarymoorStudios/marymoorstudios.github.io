...
to fit and still look good can be quite a challenge.  Layout algorithms can come in handy here.
...
2. Where each element `e_i` defines a required minimum constraint: `{ m_1, m_2, ... m_n }` (such that a value of `0`
   means _no minimum_),
...
2. If `w` is smaller than the sum of the minimums ($`\left w <= \right\sum_{i=1}^n m_i`$) then give each element its
   minimum.
3. All elements are given _equal weight_ is assigning available space.
  ...
  internal static void ComputeFill(double available, Span<double> mins)
  {
    ...
    switch (count)
    {
      case 0:
        ...
        return;
      case 1:
        // If only one element, then it always gets everything (or its min, which is larger).
...
> higher than the smallest element in group (2), then move that element to group (1) and try again.  Stop when either:
> we find a subset for group (1) that satisfies the above observations, or group (2) is empty.  This MUST converge
> because group (2) gets smaller with each iteration and so eventually group (2) MUST become empty (if we don't break
> earlier).
...
you plan on leveraging an existing layout implementation provided by your favorite game engine or framework, it is often
useful to have an understanding of what is happening under the hood.  I hope this is useful for you.  Until next time,
