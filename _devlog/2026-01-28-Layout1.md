...
date: 2026-01-28 10:24:00 -0700
bg: white
color: black
excerpt: "Using layout to stretch and cover an expanse."
featured: "true"
---
# Stretch Layout
--------
...
hardware.  All of its 256 levels were hand-coded to fit in a 28x36 grid of 8x8 tiles.  Today modern games have to run on
an incredibly wide variety screen sizes.  The same game might run on PCs, consoles, handhelds, cell phones, and even VR.
Game UI has to be dynamic to fit the screen resolution available.  This often means that the size of some game elements
need to stretch to fill a space while others are kept fixed.  Many elements, like buttons or labels, need to put minimum
constraints on their size to ensure they are still readable and/or clickable.  Getting everything to fit and still look
good can be a challenge.  This is where different layout algorithms come in handy.
...
[VBoxContainer][godot-vbox-container].  Some details may differ between implementations, but the underlying problem
space is the same.  A simple statement of the problem might be:

1. For a set of `n` elements `{ e_1, e_2, ... e_n }`.
2. Each element `e_i` defines a required minimum constraint: `{ m_1, m_2, ... m_n }`.  A value of `0` means _no
   minimum_.
3. **Solve:** Arrange all elements to fill a spanning distance `w`, such that:
4. All elements receive **_at least_** their minimum, and otherwise
5. Evenly distribute space between all elements **equally**.

This problem is deliberately underspecified, and there can be several interesting corner cases and additional
constraints.  For the purposes of this post, we will assume the following additional simplifying assumptions:

1. Elements have no _maximum_ size constraint.
2. If `w` is smaller than the sum of the minimums ($$\left w <= \right\sum_{i=1}^n m_i$$) then give each element its
   minimum.

Our desired algorithm will solve for the widths (or heights if the distance is arranged vertically) assigned to each
element.
...
title "Stretch Layout"
y-axis 0 --> 100
x-axis [e_0, e_1, e_2, e_3, e_4]
bar  [10, 40, 30, 60, 20]
```

The available space we need to span `w` can be thought of as the total amount of water that we have to work with.  Think
of this remaining water in a pitcher ready to pour.  To start with, we fill all of the separate `e_i` containers with
water all the way up to their minimums:
...
simplifing assumption (2) above).  However, if water remains, then we start filling uniformly by pouring water into the
outer box causing the surrounding water level to rise.
...
surrounding water, while any containers remaining above the water level will get their (still larger) minimum values.

## Algorithm

So, let's code this us.  Let's write a function that, given an available spanning distance and a set of item minimums,
will compute a stretch to fill layout.

First we need to take some arguments, and check some base conditions:

```cs
  /// <summary>Computes the minimum spanning distances for a set of items with minimums.</summary>
  /// <param name="available">The total available spanning distance to cover with all items.</param>
  /// <param name="mins">On input, the required minimums of each input.</param>
  /// <return>On output, <paramref name="mins"/> is updated to the suggested spanning distance of each item.</return>
  internal static void ComputeFill(double available, Span<double> mins)
  {
    ...
    switch (count)
    {
      case 0:
        ...
        return;
      case 1:
        // If only one item, then it always gets everything (or its min, which is larger).
...
entry and the assigned values on exit (to avoid allocating).  Next, we need to figure out if there is any space left
over after filling the minimum requirements.  If not, then there is nothing more to do:
...
If there is space left over after satisfying all minimums, then we can make the following observations based on our
analogy above.  Whatever the final shared water level is, all of the elements can be partitioned into one of two
sets:
...
> higher than the smallest element in group (2), then move that element to group (1) and repeat.  Stop when either: we
...
```cs
    ...
    {
      // Assume first k items are at `level` (>= their mins),
      // and the rest (`k` through `count-1`) are clamped at their mins.
      level = (available - postfix[k]) / k;

      // All items share the same `level` (no higher mins left).
      if (k == count)
      {
        break;
      }
      ...
      // the first k items can all be at `level`, and the rest at their mins.
      ...
      // Otherwise, `level` is above nextMin, meaning the next item should also be raised.
...
1. We sort the elements ascending by their minimums to make it easier to find the smallest item in group (2) on each
   iteration.
2. We pre-compute into the postfix array the sum of all minimums to the right of each position (including that position
   itself).  At the pivot `k`, then all elements to the right are going to get their minimums, so we will subtract the
   sum of those minimum from `available` *before* computing the shared level for the elements in group (1) to the left
   of `k`.

Finally, we make the assignments.  From the loop above we _know_ what the shared level is going to be.  Therefore, all
...
unit of the shared level assigned to other elements, _that_ element would receive _two_ units.

## Conclusion
In this is post, we looked at a **Stretch to Fill** algorithm.  Algorithms like this help game designers build dynamic
UI that adapts itself automatically to different screen sizes and resolution while still remaining functional.  Even if
you can level and existing implementation provided by a game engine or framework, it is often useful to have an
understanding of what is happening under the hood.  I hope this is useful.  Until next time, keep your head above water,
and code on!
