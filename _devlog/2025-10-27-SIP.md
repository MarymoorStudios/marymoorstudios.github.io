...
One of the primary problems with running multiple concurrently programs quickly became obvious on these early
multiprogramming systems.  Two programs running concurrently can write all over each other's memory.  If two programs
try to use the same piece of memory concurrently, they are likely to corrupt each other's operation.  For two programs
to safely run concurrently they must agree ahead of time to use different, non-overlapping, segments of memory.  This
makes software design more difficult and negatively impacts composability.
...
their own address tables into the MMU before they accessed memory _and_ their tables mapped that vritual address to
...
is mediated by the OS and whose access to a strict subset of physical memory is enforced by the MMU hardware under the
exclusive control the OS (not the program).

Once again, each program was given _the appearance_ of executing strictly sequentially on a single CPU with exclusive
access to all of its addressable memory.  A program could safely execute concurrently in this environment along side any
number of other programs without being concerned by conrruption or interference.  This made it considerably easier for
...
1960's sported two or more CPUs.  By the 1970s many commerical systems supported multiple CPUs configurations arranged
either as symmetric multiprocessing (SMP) systems where all CPUs share access to all the memory, or as message-passing
systems where programs running on different CPUs send messages to each other to coordinate the access to CPU-isolated
memory.
...
programs in that they executed their own independent program logic concurrently (or in parallel) with each other.
...
make multiple copies.  It means all of the threads can contribute to a shared computed solution for a large computing
...
multithreaded programming model includes the complexities of synchronization primitives (locks) to help to address these
issues.  Despite being intensely popular (and perhaps seen as a badge of prowess) the proper use of locks can be
...
of looking at and thinking about the logic executing in a program (particularly one with multple threads).  A simple
...
logging or debuggging).  The logic and state can be independently reasoned about making it easy to test independently of
the larger application that uses it.  Multiple instances of the same SIP can be instantiated at the same time, each with
independent identity and an independent copy state.  This makes a SIP reusable with high composability.
...
processes.  Let's look at some of the requirements a SIP needs to meet to achieve its properties.
...
programming disciplines.  In particular:
...
2. **No Mutable Statics:**  
   ...
   Immutable statics are invaluable as constants and compile-time defined data structures.  In a multithreaded
   ...
   composability, testing, separation of concerns, and maintenance.  So I tend to recommend this precept independently
   of SIPs anyway.)
3. **Communicate Only By-Value or Linear Transfer:**  
   When a SIP sends a message to another SIP, the message content itself MUST not introduce new references to shared
   objects.  The easiest way to guarantee this is communicate only through memory copying (i.e. by value).  A good
   serialization library can help efficiently flatten complex structures into an easily copiable sequence of bytes. Then
   a trusted piece of code can perform a blit (aka byte block transfer) from the sending SIP's memory to the receiving
   SIP's memory.  Alternatively, for very large or complex object trees, a [Linear Transfer][define-linear] could be
   imployed.  A warning: for a linear transfer to be applicable the entire object tree to be transfered must itself be
...
4. **Memory Safety Must Be Obeyed At All Times:**
   Obviously, direct unsafe memory accesses have the potential to violate memory isolation.

Almost any programming language can be used to implement a SIP, but some will demand greater developer discipline than
others.  Some will provide better guarantees through compile-time or run-time enforcement.  How easy these requirements
are to implement will greatly depend on the programming language you use, your personal style, and the operating system
you have to work with.  

Memory-managed languages like C#, Rust, Go, or Java can make it significantly easier to achieve the memory isolation
properties describe above.  Their compilers can enforce (1) and (4) at compile-time through their type systems.  (3) can
be guaranteed by-construction through serialization and materialization.  Only (2) relies on developer discipline.  (In
some environments even (2) can be enforced at compile-time through additional tooling, such as C#'s roslyn-based
analyzers which can detect and flag the unintential uses of mutable statics.)

Even in environments as inhospitable to memory satefy as C++ is, I have had great succeess in utilizing the SIP paradigm
through the judicious use of modern C++ primitives and some developer care.  IMHO the benefits have always vastly
outweighed any cost in expressability.

## Conclusion
This is post, we talked about the Software Isolated Process or SIP as a programming methodology that promotes disecting
complex multithreaded programs into independent segments of logic and state which can be executed sequentially, each
with their own exclusive access to a block of isolated memory.

The benefits of SIPs include:
  * Freedom from the complexities and subtlies inherent in the use of locks.
  ...
  * Easier cleanup and recovery due to isolation and bounded SIP lifetimes.
