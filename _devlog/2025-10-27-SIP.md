...
of its state is stored in (and _only_ in) a segment of memory that only it has access to guaranteeing that state is also
...
3. **Communicate Only Through Explicit Channels:**  
   ...
   or whether you are crossing process or network boundaries.  A communication channel could be as simple as a
