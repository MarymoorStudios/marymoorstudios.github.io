...
number of other programs without being concerned with conrruption or interference.  This made it considerably easier for
...
1960's sported two or more CPUs.  By the 1970s many commerical systems supported multiple-CPU configurations arranged
...
multithreaded programming model includes the complexities of synchronization primitives (locks) to try to address these
...
scripting to modern microservices.  The SIP methodology applies all of these same ideas but to the multiple threads of
concurrent execution that make up complex modern software applications (especially and including games).
...
3. **Communicate Only Through Explicit Channels:**  
   ...
   even whether you are crossing process or network boundaries.  A communication channel could as simple as a
   thread-safe [queue][concurrent-queue] or as sophisticated as an [RPC session][background-sip].  The key principle is
   that it only allows explicit, structured, message exchange.  It MUST not allow either party unstructured access to
...
4. **Communicate Only By-Value or Linear Transfer:**  
   ...
   Alternatively, for very large or complex object trees, a [Linear Transfer][define-linear] could be imployed.
   **Warning**: for a linear transfer to be applicable the entire object tree to be transfered must itself be
...
which can detect and flag the unintentional uses of mutable statics.)
...
disecting complex multithreaded programs into independent segments of logic and state which can be executed
