...
* **States:**  
  ...
  (or intializer) determine the starting state.
...
2. The properties of an input event (i.e the arguments to the input methods).

### Variable Conditionals
Let's extend the `Process` input method to take _time delta_ in wall-clock time since the last frame was rendered.
(This is a very common technique used by game engines to align handler behavior to the passage of real time without the
use of explicit timers - which are expensive.)  Consider this extended example:
...
* A text parser might accumulate characters until a non-symbol character is reached, at which point the parser makes a
  state transition from the symbol accumulation state to grammar state that will consume the new symbol token.
...
Suppose we need to occasonally take our light out of service for maintenance.  While in maintenance the light should
...
In the new `MaintenanceMode` input the input parameter `enable` doesn't just modify state variables, but directly
impacts the state transition.  (FSMs that trigger output based on both their state and their input parameters are
sometimes called a [Mealy][mealy] machines.)
...
On1, ... OnThreshold ]` that represent the combined semantic meaning of being both in the `On` state have having
accumulated `n` seconds of time (so far).  Its easy to see that this alternative formulation, while perhaps more
difficult to write in code due to its verbosity, would be equivalent computationally.
...
* Using more variables generally leads to a reduction in the total number of explicit states, but makes coding input
  handlers more complex (and therefore more likely to have bugs).  Therefore, minimizing variables generally leads to
  more correct code.
* Use explicit states for all _modal_ behavior changes.  That is, if the object has two "modes" of behavior (i.e.
  ...
  states.
* Use variables for accumulators (e.g. numeric or string values) that would otherwise lead to an explosion of explicit
  state space.

## Nested FSMs
One of the reasons that I like this particular formulation of FSMs is that it easily facilitates nested FSMs. Logically,
a nested FSM is strictly speaking just a larger FSM (i.e. by combining their states, inputs, and variables together with
their parent's). In practice, however, nested FSMs provide a powerful abstraction for the separation of concerns between
state machines, and for reusability.  For example: 
...
* In a parser, many different grammar productions need to parse a symbol.  The symbol FSM might be nested within them.

Consider this expanded Traffic Intersection example which uses 4 traffic lights as nested FSMs (stored in a state
variable):
...
both the `MaintenanceMode` and `Process` inputs.  The smaller `TrafficLight` FSM manages only the blinking of the light
and relies on its parent for maintenance status.  The `TrafficLight` FSM only has a single `Process` input, having no
behavior for the `MaintenanceMode` input.  The `TrafficLight` FSM is both nested and multi-instanced within its parent
`TrafficIntersection`.  The `TrafficLight` abstraction has higher reusability than it would if it were directly merged
into `TrafficIntersection`.  It could be used to build standalone lights, two light intersections (like a pedestrian
crossing), or the 4-way seen here.

## Conclusion
In this is post, we reviewed Finite State Machines.  We looked at the basic **Enum-FSM** design pattern.  This design
pattern is simple, clean, and easy to implement in most modern programming languages.  We examined a few of the
...
which scale better for larger machines.  Until next time, code on!
