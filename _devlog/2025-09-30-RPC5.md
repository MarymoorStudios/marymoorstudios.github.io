...
[Promise RPC][promise-rpc], like many message passing systems, interacts with two different layers of abstraction during
point-to-point communication.  Each is responsible for different aspects of the overall implementation.
...
* **Session Layer:**  
  ...
  for authentication, version, and configuration negotiations.
...
tracking (such as determining if delivery was successful) must be implemented by a _higher_ layer.  Alternatively, a
...
Networking is a transport with multiple implementations over many different transports including UDP and TCP.
...
* **Control Messages:**  
  ...
  messages may appear directly in the main application ordering, or they may be sent and received using an alternate
  ordering that is independent of application interactions.  When sent using an alternate ordering they are sometimes
  called out-of-band (OOB) messages.
...
* **(Application) Messages:**  
  ...
  passing system.  These are those _messages_.  A message passing system usually works in conjunction with a
...
Though there are many Transport implementations, there is only a single Session implementation.  The Session is what
separates one message passing system from another.  In a program, the message passing system maintains a set of tables
representing the state of the communiciation between all of the connected parties including a collection of zero or more
session instances.  Each session instance represents an active communication between the current process and another
party.  Each instance consists of two endpoints (one representing the current process and the other its remote peer), a
transport (that provides a mechanism for the two endpoints to communicate bidirectionally), and any additional state
that is unique to the session.
...
2. The version of the session protocol to use (if there is more than one available) MUST be established.  If the two
   ...
   case, the two parties will identify and agree on which to use for this session.
...
terminated automatically in the context of a larger semantic scope with a program.  For instance, when another SIP is
launched in the same process then a session between the current SIP and the new SIP is established automatically and the
coresponding root capability is returned to each.  Similarly, if a child process is launched then a session between it
and its parent is established automatically by the process creation library.  A SIP represents a kind of _scope of
sessions_, each connecting that SIP to some other SIP.  When a SIP terminates then, natually, all of its sessions are
also terminated at the same time, automatically.
...
1. **Void Promises:** track the completion of an activity.  They have only a `void` value, but may resolve to an error
   if the computation failed.
2. **Data Promises:** both track the completion of an activity and resolve to a data value that a computation produces
   as its return value.  They may resolve to an error if the computation failed to produce a result.
3. **Proxy:** both track the completion of an activity producing a capability and provide access to the methods of that
   capability.  
   
We can pipeline additional computations by using `async` activities and the `await` keyword (or the `When` method) on
any promise type.  An `async` activity returns another eventual value as its return value.  Similarly, proxies allow for
pipeling method calls immediately upon their creation (even before the proxy has been resolved) producing additional
eventual values as the return values of those method calls.  The composition of these speculative computations form
directed graphs of eventual computation whose edges execute on-demand as the eventual values forming its nodes resolve
in time.

An eventual value spans a session when a consumer of that value lies on one side of a session and the producer of the
result lies on the other.  A capability may continue to span a session (even after the activity that produced it has
completed) if the target object it references lies on the other side (i.e. is remote).  When an eventual value is
consumed on this side of the session and produced on the other, we call it an _incoming_ value.  When produced on this
side of the session and consumed on the other, it is called an _outgoing_ value.

When a session is terminated then no _incoming_ value that spans that session can ever be resolved since there is no
longer any way to communicate the outcome.  All such _incoming_ eventual values are therefore immediately _Aborted_.  An
_abort_ is an error outcome similar to cancellation that resolves outstanding eventual values to the well-known
[`AbortedException`][docs-aborted].  Similarly, all _incoming_ capabilities that span that session are _Revoked_.  A
_revoked capability_ will produce aborted results for all subsequent method calls.

In this way any pending speculative computation graph will _come to know of_ the aborts (and the session termination).
Aborts can be explicitly handled, as with any error, identified by their unique exception type, but often all errors are
allowed to automatically coalesce and collapse the computational graph until they can be handled (or written to logs for
diagnostic purposes) much higher up on the stack.  Coalescing aborts and errors generally simplifies programming and
improves correctness by reducing the total number of states a program must properly handle.

## Conclusion
In this post we talked about transports and session.  We examined how sessions are created and what kind information is
exchanged during session initiation.  We looked at various forms of session grouping and lifetime scoping.  Lastly, we
looked at session termination and how that impacts eventual values that span sessions.
