...
In this way any pending speculative computation graph will _come to know_ of the aborts (and so the session
termination).  Aborts can be explicitly handled, as with any error handling, but often errors are allowed to
automatically coalesce and collapse the computational tree until they can be handled much higher up on the stack (or
written to logs for diagnostic purposes).  Coalescing aborts and errors generally simplifies programming by reducing the total number of states and program must properly handle.
...
backpressure.  Lastly, we looked at some unique characteristics of the [Promise RPC][promise-rpc] library's
