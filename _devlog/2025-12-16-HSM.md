...
> [!NOTE]  
> [Professor Liskov][liskov] was my teacher in undergraduate studies and my graduate adviser. She, more than
> anyone, helped me learn to appreciate type systems and programming methodology.

## State Diagrams
You'll notice that throughout my posts about FSMs I have used _state diagram_ to both declare and visualize the state
machines that we have been working with.  In this blog I use [mermaid][mermaid] to declare the FSM in text, such as
this:
...
While the visualizations are nice, for me, the real power of these diagrams lies more in the declarative nature of the
syntax used to produce them.  Using declarative language to describe, at a very high level, the structure of an FSM
forces me to think about its design.  In the declaration you can see all of the states together on one page (not spread
across the code in different methods).  You can see the intended transitions between them and which inputs will cause
them.  I always do the design first, and then code it.  You don't even need fancy tools like [mermaid][mermaid] to
declare an FSM design.  For simple FSMs, often I'll just write the declaration in simple text comments at the top of the
class, using any syntax I choose.  As long as it is clear, it can be good enough.

One of the benefits of an FSM declaration is that you can easily see which inputs are allowed to transition between
which states.  If you are a firm believer in invariants and asserts (and you know that I am), then you might find it
valuable to (liberally) sprinkle asserts in your input dispatch methods to validate that ONLY validate state transitions
occur.  These invariants often catch bugs either in the code (where a handler transitioned to the wrong state) or in the
design (where an interaction was not properly accounted for).
...
software, and FSMs are not exempt.  Reentrancy hazards are a common occurance in games due to the frequent use of FSMs.
How can unintentional or unexpected reentrancy lead to bugs?
...
Now, imagine that our Enemy has some kind of shield or armor that reflects a portion of damage delt (e.g.
...
```cs
  private void EnemyTakeDamage(Hsm data, int damage, Hero attacker)
  {
    ...
    if (data.m_reflectDamagePerc != 0) 
    {
        attacker.TakeDamage(damage * data.m_reflectDamagePerc);  // Reflect some damage back.
    }
  }
```

When the player presses the `Attack` button near an Enemy, then `Hero.Attack` will be called triggering the `Attack`
handler above which will then call `Enemy.TakeDamage` which will in turn trigger the `EnemyTakeDamage` handler, which
will (finally) call `Hero.TakeDamage` which will _then_ trigger the `TakeDamage` handler.  Reentrancy!  We are STILL in
the middle of executing the hero's `Attack` handler, but we have re-entered the Hero FSM to execute its `TakeDamage`
handler.
...
OK, so what?  The code will complete (i.e. no infinite recursion or anything here).  Why is this bad?  Well, suppose the
...
Reasoning).  Even if the code doesn't exhibit this kind of reentrancy when it is written, the reentrancy could easily be
accidentally introduced in a subsequent modification.  For instance, perhaps the `thorns armor` feature didn't exist in
V1, but was introduced later in an update.
...
`Downed` state is no longer lost.  BUT, the reentrancy will lead to the Hero to be in the `Downed` state after the
`Attack` handler returns.  BUT, the `Attack` input has no valid transition to the `Downed` state.  If we had added
transition validation asserts, then they would fire because this is a transition not covered in the state diagram!
Again, we could continue to pull the thread of hacks by adding the `Downed` state transition to the `Attack` state, but
you can quickly see that this ultimate makes the set of valid transitions almost impossible to reason about.  To know
which transitions are valid it is no longer enough to just look at the state diagram, instead you need to understand
every interaction between handlers leading us back to Global Reasoning.

## Deferred Input Reentrancy Execution
What is the proper solution?  Deferred Input Reentrancy Execution (DIRE).  Like we saw with the async message dispatch
model, we need to defer executing additional handlers for inputs triggered during an already executing handler.  I.e. we
MUST restore the [Top of Turn property][define-tot] before dispatching additional handlers.  DIRE gives us both
correct-by-construction execution AND guaranteed Local Reasoning.  Regardless whether a handler is triggered directly or
reentrantly, its view of the variables (including the `m_current`) is a **Top of Turn** view, and any state transitions
can be reasoned about (and validated) on a per-handler basis &mdash; just as they are depicted in the _state diagram_.

Implementing DIRE can be _complex_.  A full description of doing so is behond the scope of this post.  So, I'll leave
that as an exercise for the reader. &#x1F61C;  However, if you want to see a full implementation of DIRE in action,
along with a C#-based FSM declaration language, automatic code generation of HSM boilerplate, and a bunch of other
advanced features then check out our new FSM library addition to [MSC][MSC] called [MarymoorStudios.Core.Fsm][fsm-lib].
...
## Conclusion
In this is post, we completed our discussion **Finite State Machines**.  We looked at the more advanced **HSM** design
pattern.  We talked about _state diagrams_.  And lastly, we examined additional dangers of the dreaded reoccuring
