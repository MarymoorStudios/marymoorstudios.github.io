...
named singleton constant (instead of the enum constants we saw in the **Enum-FSM** pattern).  We then leverage the
...
[Top of Turn][define-tot] property in message ordering.  Reentrancy is a hazard, _not only_ to RPC messaging, but to all
...
handler.  I.e. we MUST restore the [Top of Turn][define-tot] property before dispatching additional handlers.  DIRE
...
recently added to the [MSC][MSC] suite.
...
## Next
Read the [next post][devlog-post12] in this series.
