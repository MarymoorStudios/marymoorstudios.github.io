...
rooms defined at compile-time.  We want the system to be able to create (and destroy) new chat rooms at will, but we
still want to get ordering guarantees.  What we want is more than one **Instance** of the _same_ Interface.  Then we
...
and see if our capability solution addresses any of them.
  ...
  5. **During message dispatch, how do you find the data structure that manages that specific room's data?**  
     ...
     and (3) above.  Table access MUST be very cheap, because the table would need to be consulted explictly during each
     dispatch to convert the object id argument into a reference to the associated state.
...
session.  So it is possible that different sessions to the same endpoint export different root capabilities based on the
...
calling the `TryJoin` method and passing: (1) a `slotId` which identifies the desired slot to claim, and (2) a
...
and bytes, and finally we'll see how session lifetime relates to aborts and cancellation. Until next time, code on!

## Previous
Read the [previous post][devlog-post4] in this series.

## Next
Read the [next post][devlog-post6] in this series.
